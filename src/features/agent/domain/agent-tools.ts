...
export const MANAGER_ONLY_ROOMS = ['Переговорка «Орион»']

export function roomsForRole(role: AgentRole): string[] {
  if (role === 'manager') {
    return ROOMS
  }
  return ROOMS.filter((room) => !MANAGER_ONLY_ROOMS.includes(room))
}
...
> = {
  bookMeetingRoom: (store, clock) => async (args, identity) => {
    ...
    if (!room) {
      ...
      room =
        roomsForRole(identity.role).find(
          (candidate) =>
            !dayBookings.some(
              (booking) =>
  ...
  listAvailableRooms: (store) => async (args, identity) => {
    const date = pickString(args, 'date')
    const time = pickString(args, 'time')
