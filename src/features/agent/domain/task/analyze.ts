...
const ANALYSIS_RULES = [
  ...
  '- Переход "planning" → "execution" возможен по явному согласию пользователя («да», «приступай», «всё верно», «окей делаем»), если план содержит изменяющие действия, а не из-за того, что параметров достаточно. Для чисто справочных задач (только list*: показать встречи, расписание, отпуска) согласие не требуется — их можно сразу ставить в "execution".',
  '- Переход "planning" → "execution" возможен только когда план полностью определён: комната, дата/время, длительность и участники названы пользователем или явно им подтверждены.',
  '- Не подставляй значения по умолчанию для обязательных параметров молча. Если что-то не названо — оставайся в "planning", перечисли открытые пункты в "steps" и поставь expectedAction.actor = "user".',
  '- Не считай существование объекта предусловием его создания: чтобы создать новое расписание, бронь или заявку, не нужно, чтобы объект уже был в списках. Справочные list* нужны для фактических данных, а не для проверки «можно ли создать».',
  '- Не придумывай параметры, которых нет у инструмента. Для сбора погоды значимы только город, периодичность (минуты) и окно (часы): если пользователь их не назвал, предложи дефолты (каждые 60 минут, окно 1 час, старт сейчас) и попроси подтвердить план целиком — не спрашивай формат, получателя или период отчёта.',
  '- Перед "execution" в "steps" должен быть полный список пунктов плана; меняй этап только по сообщению, утверждающему весь план.',
  '- Шаги этапа "execution" — это изменяющие действия (бронь, приглашение, отмена и т.п.). Не выделяй отдельным шагом справочные проверки: сверка результата относится к этапу "validation".',
