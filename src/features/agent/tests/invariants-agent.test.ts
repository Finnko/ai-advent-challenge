...
describe('agent invariants enforcement', () => {
  ...
  it('blocks an employee booking Orion but lets a manager book it', async () => {
    const orion: InvariantRecord = {
      ...invariant,
      id: 3,
      slug: 'orion-employee',
      title: 'Орион',
    ...
    const decision =
      ...
      async ({ response_format }) => ({
        content: response_format ? JSON.stringify({ tool, args }) : 'Готово.',
        usage: null,
        latencyMs: 0,
      })

    const employeeStore = createFakeStore()
    const employeeRun = await new Agent({
      capabilities: createCapabilities(employee, ['bookMeetingRoom']),
      tools: createAgentTools(employeeStore, TEST_NOW),
      judges: [],
      callLLM: decision('bookMeetingRoom'),
      model: 'test',
      today: '2026-09-10',
      invariants: [orion],
    }).run('Забронируй Орион на 16:00')
    expect(employeeRun.blocked).toBe(true)
    expect(employeeRun.invariantHits).toEqual(['INV-3'])
    expect(employeeStore.bookings).toHaveLength(0)

    const managerStore = createFakeStore()
    const managerRun = await new Agent({
      capabilities: createCapabilities(createManagerIdentity(), [
        'bookMeetingRoom',
      ]),
      tools: createAgentTools(managerStore, TEST_NOW),
      judges: [],
      callLLM: decision('bookMeetingRoom'),
      model: 'test',
      today: '2026-09-10',
    ...
    expect(managerRun.blocked).toBe(false)
